...
namespace TTGamesExplorerRebirthLib.Formats.DDS.BCnEncoder.Net.Decoder
{
    ...
    public class BcDecoder
    {
        ...
        /// <param name="format">The compression format used.</param>
        /// <returns>The decoded 4x4 block.</returns>
        public static Memory2D<ColorRgbFloat> DecodeBlockHdr(ReadOnlySpan<byte> blockData, CompressionFormat format)
        {
        ...
        /// <param name="format">The compression format used.</param>
        /// <param name="outputSpan">The destination span of the decoded data.</param>
        public static void DecodeBlockHdr(ReadOnlySpan<byte> blockData, CompressionFormat format, Span2D<ColorRgbFloat> outputSpan)
        {
            if (outputSpan.Width != 4 || outputSpan.Height != 4)
            {
                throw new ArgumentException($"Single block decoding needs an output span of exactly 4x4");
        ...
        private static void DecodeBlockInternalHdr(ReadOnlySpan<byte> blockData, CompressionFormat format, Span2D<ColorRgbFloat> outputSpan)
        {
            var decoder = GetRgbFloatDecoder(format);
            if (!format.IsHdrFormat())
            {
        ...
        private static IBcBlockDecoder<RawBlock4X4RgbFloat> GetRgbFloatDecoder(GlInternalFormat format)
        {
        ...
        private static IBcBlockDecoder<RawBlock4X4RgbFloat> GetRgbFloatDecoder(CompressionFormat format)
        {
            return format switch
            {
        ...
        public static int GetBlockSize(CompressionFormat format)
        {
            return format switch
            {
                ...
                CompressionFormat.Bc1 or CompressionFormat.Bc1WithAlpha => Unsafe.SizeOf<Bc1Block>(),
                ...
                CompressionFormat.Bc6S or CompressionFormat.Bc6U => Unsafe.SizeOf<Bc6Block>(),
