    ...
    public class BcEncoder
    {
        ...
        public int GetBlockSize()
        {
            ...
            if (compressedEncoder == null)
            {
                ...
                return hdrEncoder switch
                {
                    null => throw new NotSupportedException($"This format is either not supported or does not use block compression: {OutputOptions.Format}"),
                    _ => hdrEncoder.GetBlockSize()
                };
            }
        ...
        public static void CalculateMipMapSize(int imagePixelWidth, int imagePixelHeight, int mipLevel, out int mipWidth, out int mipHeight)
        {
        ...
        {
            KtxFile output;
            Bc6Encoder compressedEncoder = null;
        ...
        private byte[] EncodeBlockInternalHdr(ReadOnlySpan2D<ColorRgbFloat> input)
        {
            ...
            if (compressedEncoder != null)
            {
                ...
                input.GetRowSpan(1).CopyTo(pixels[4..]);
                input.GetRowSpan(2).CopyTo(pixels[8..]);
                input.GetRowSpan(3).CopyTo(pixels[12..]);
        ...
        private void EncodeBlockInternalHdr(ReadOnlySpan2D<ColorRgbFloat> input, Stream outputStream)
        {
            ...
            if (compressedEncoder != null)
            {
                ...
                output = output[..compressedEncoder.GetBlockSize()];
        ...
        private static Bc6Encoder GetFloatBlockEncoder(CompressionFormat format)
        {
            return format switch
            {
                CompressionFormat.Bc6S => new Bc6Encoder(true),
                CompressionFormat.Bc6U => new Bc6Encoder(false),
                _ => null,
            };
        }
        ...
        private static ReadOnlyMemory2D<ColorRgba32> ByteToColorMemory(ReadOnlySpan<byte> span, int width, int height, PixelFormat format)
        {
