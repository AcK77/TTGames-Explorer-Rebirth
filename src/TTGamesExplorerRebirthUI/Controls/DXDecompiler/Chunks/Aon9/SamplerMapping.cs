...
namespace DXDecompiler.Chunks.Aon9
{
    public class SamplerMapping
    {
        ...
        public static SamplerMapping Parse(BytecodeReader reader)
        {
            ...
            {
                SourceResource = reader.ReadByte(),
                SourceSampler = reader.ReadByte(),
                TargetSampler = reader.ReadByte()
            };
        ...
        {
            return string.Format("// s{0, -13} s{1, -14} t{2, -16}", TargetSampler, SourceSampler, SourceResource);
        }
    }
}