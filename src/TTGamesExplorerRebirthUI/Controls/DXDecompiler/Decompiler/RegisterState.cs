...
namespace DXDecompiler.Decompiler
{
    ...
    {
        private readonly BytecodeContainer Container;
        readonly Dictionary<string, List<Register>> Registers = [];
        //Contains types used by structured buffers (struct/scalar/matrix)

        readonly Dictionary<string, ConstantBuffer> ResourceBindings = [];
        ...
        {
            if (Registers.TryGetValue(key, out List<Register> value))
            {
                value.Add(register);
            }
            else
