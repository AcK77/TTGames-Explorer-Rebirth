...
namespace DXDecompiler.Decompiler
{
    ...
    {
        static string GetOperandIndex(Operand operand)
        {
        ...
        static string GetSwizzle(Operand operand, ComponentMask mask)
        {
            /*
            var parentType = operand.ParentType;
            var numComponents = operand.NumComponents;
        ...
        internal static ShaderTypeMember GetShaderVariableByOffset(ConstantBuffer cb, uint offset, out string fullname)
        {
            foreach (var variable in cb.Variables)
            {
                var result = IsOffsetInType(variable.Member, 0, offset, out fullname, out bool isArray);
                if (result != null)
                {
                    return result;
                }
        ...
        internal static ShaderTypeMember IsOffsetInType(ShaderTypeMember member, uint parentOffset, uint offsetToFind, out string fullName, out bool isArray)
        {
