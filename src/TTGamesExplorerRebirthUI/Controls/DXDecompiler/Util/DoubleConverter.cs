    ...
    public static class DoubleConverter
    {
        ...
        public class ArbitraryDecimal
        {
            ...
            public string ToString(int requestedPrecision = 6)
            {
                ...
                if (decimalPoint < 0)
                {
                    return new string(digitString) + new string('0', -decimalPoint) + '.' + new string('0', precision);
                }
                // Value is just below the rounding point, eg 0.0000006
                if (decimalPoint - digitString.Length == precision)
                {
                    return "0." + new string('0', decimalPoint - digitString.Length - 1) + (digits[0] >= 5 ? "1" : "0");
                }

                // Nothing before the decimal point, eg 0.035
                if (decimalPoint >= digitString.Length)
                {
                    return string.Concat("0.", (new string('0', decimalPoint - digitString.Length) +
                                                new string(digitString)).AsSpan(0, Math.Min(precision, digitString.Length)), new string('0', Math.Max(precision - digitString.Length, 0)));
                }
                // Most complicated case - part of the string comes
                // before the decimal point, part comes after it,
                // eg 3.5
                return new string(digitString, 0, digitString.Length - decimalPoint) + "." +
                       new string(digitString, digitString.Length - decimalPoint, Math.Min(decimalPoint, precision)) + 
                       ((decimalPoint <= (precision - 1) && requestedPrecision != -1) ? new string('0', precision - decimalPoint) : string.Empty);
            }
        }
    }
}