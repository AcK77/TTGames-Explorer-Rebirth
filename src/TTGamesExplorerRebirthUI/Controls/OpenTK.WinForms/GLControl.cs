    ...
    public class GLControl : Control
    {
        ...
        private unsafe void NonportableReparent(NativeWindow nativeWindow)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                IntPtr style = new((long)(Win32.WindowStyles.WS_CHILD | Win32.WindowStyles.WS_DISABLED));
                ...
                style = new((long)Win32.WindowStylesEx.WS_EX_NOACTIVATE);
        ...
        private static unsafe void EnableNativeInput(NativeWindow nativeWindow, bool isEnabled)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                IntPtr hWnd = GLFW.GetWin32Window(nativeWindow.WindowPtr);
        ...
        private static unsafe bool IsNativeInputEnabled(NativeWindow nativeWindow)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                IntPtr hWnd = GLFW.GetWin32Window(nativeWindow.WindowPtr);
